...
\paragraph{Problem.} Modularizing caches cannot be realizing without loosing performances. \ab{put an example here}


% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
\section{Refactoring}\seclabel{refactoring}
\ab{Use the present tense, not the past. I corrected some of them only.}
...
\ab{We should give a summary. A table that say how many occurrences of each pattern, quantity of methods involves in the MOGraphElement hierarchy, \% of the total Mondrian code. This will make the story stronger.}

\subsection{Refactoring Strategy}

Once the code patterns \del{were} identified, strategies to refactor them
...
it introduced indirections to access to the caches \ab{this is an important point, we are currently saying that classical composition mechanism of OO languages is not enough. This is a good point for using AOP and reflection}. Indirections caused
...
section, the code related to the cache is extra\ins{c}ted to the before and
...
value at which is initialized \chg{remaings}{remains} in the method. In the case
of the example presented in Section \ref{sub:Pattern-Identification},
the refactored code is shown below:

\ab{How do you compose the caches?}
