...
%In this case, all the cache variables and the fragments of code of the methods in which they were used were encapsulated using the \emph{Cache} class and creating a suitable \emph{CacheableItem}. 

In \emph{MOGraphElement} hierarchy only remain a reference to the \emph{Cache} class that contains all the caches\ab{I do not understand. Did I just say that?}. 

\paragraph{Significant overhead.} This \del{kind of} modularization \ins{solely based on delegating message} \chg{looses performances}{has a sever overhead at execution time}
because \ugh{the access to the cache variable is not direct}. This causes
a delay\ab{delay?} when attempting to access the values that keep the caches.
...
\ab{say how you identified the caches. Using variable accessors, sender of accessor and mutators I imagine?}

\figref{PatternLocation} shows the distribution of the caches over the main Mondrian classes. \ab{Describe the figure better}

After this initial
identification, the fragment of codes \ugh{in which} the caches are used
are grouped together based on the \ugh{purpose of its use}\ab{not really clear, what is a purpose of its use? give an example}. Each group is associated with different activities:\ab{add one sentence for each of the item below} 
\begin{itemize}
...
\sv{Cambie lo de grupo y subgrupo porque era confuso}\ab{remove these comments if they are not necessary} 
The task of group\ab{I am still confused with group here} the fragments of code of the different caches is
...
repeated in the use of the caches. For each patter\ins{n} found a refactoring strategy is defined\ab{Do you want to say that a refactoring is associated to each pattern? If yes, then just say it :-)}. These code patterns are described in the
following subsections.

%========
\subsection{Pattern Description\label{sub:Pattern-Identification}}

\ins{We identified 5 code patterns based on Mondrian source code and are described below.}
\del{This section presents the 5 code patterns we identified.} 
Each pattern is described with a relevant typical occurrence,
the number of occurrences \chg{of the pattern}{we found in Mondrian} and \chg{a code example}{an illustration}.

\paragraph{Reset Cache.} A cache \chg{need}{has} to be invalidated when its content has to be actualized. We refer to this action as reset. The reset structure is \emph{cache:=resetValue} where
\emph{resetValue} depends on the cache internal structure. Typically,
the \emph{resetValue} \chg{is nil or a new of a kind of Dictionary object}{depends on the type of the stored value. It could be \ct{nil}, an empty dictionary, or a particular value (\eg \ct{0@0})}.
Eighteen occurrences of this pattern are found in \del{the} Mondrian\del{ code}. 
We found that in some occurrences the reset of the caches is performed before the logic of the method, and other methods in which the reset must be done after\ab{gives an example. This is important}. 

\chg{Next, the method \emph{resetCache} is presented as an example.}{Consider the method \ct{MOGraphElement>>resetCache}. This method is called whenever the user drag and drop a graphical element.} In
...
\paragraph{Lazy initialization.} In some situations it is not relevant to initialize the cache before it is actually need. \ins{This happens when a graphical element is not outside the window. These caches are relevant only when the user actually see the element by scrolling the visualization.} This pattern shows
...
is nil\ab{I do not understand this sentence. I think it can be removed}. Typically, the structure of this pattern is: \emph{if (cache==nil)
cache:=newValue. return cache} \ab{Use Smalltalk code}. \chg{Five occurrences of this pattern are
found in the Mondrian code}{Mondrian contains five occurrences of a lazy cache initialization}. \chg{Next, the method \emph{bounds} is presented
as an example in which boundsCache is accessed.}{Consider the \ct{bounds} method:}
...
\ins{The circumscribed rectangle is returned by \ct{computeBoundsFor:} and is performed only when a edge is actually visible (\ct{bounds} is used in \ct{drawOn:} , the rendering method.}
...
in three occasions. \chg{Next, the method \emph{cacheCanvas} is presented
as an example in which a value is assigned to cacheForm.}{Consider the method \ct{cacheCanvas:}}
...
\ins{The method \ct{cacheCanvas:} is invoked when...}\ab{please complete}
...
Additionally, Table \ref{tab:Cache-Concern-scattering} \ab{something wrong with the ref?} presents a
