...
are grouped together based on the purpose of its use. \chg{As mentioned
in the previous section, the main general purposes identified are}{A cache is associated with the following actions}: 
\begin{itemize}
...
This section presents the 5 code pattern\ins{s} we identified 
Each pattern is described with \chg{the most}{a} relevant typical occurrence,
the number of occurrences of the pattern and a code example. \del{The purpose
of the code examples is to illustrate the occurrences of each pattern
in the proper context.}

\paragraph{Reset Cache.} \chg{this pattern represents a situation in which a
cache is reset}{A cache need to be invalidated when its content has to be actualized. We refer to this action as reset.} The reset structure is \emph{cache:=resetValue} where
...
We found that in some occurrences the reset of the caches is performed before the \chg{computations executed in}{the logic of}  the method, and other methods in which the reset must be done \chg{at the end}{after}. 
...
\paragraph{Lazy initialization.} \ins{In some situations it is not relevant to initialize the cache before it is actually need. } \chg{t}{T}his pattern shows
...
Fig. \ref{fig:Pattern-locations-in} shows \chg{the cache in which methods the patterns
were found}{the distribution of the caches over the main Mondrian classes}.%
\begin{figure*}
\begin{centering}
...
are established. \ab{What is the goal of the refactoring? We said it a bit in the introduction, but we need to be clear here.} The main constraint to refactoring the code is the
...
The refactoring strategy used is: for each method that contain\chg{ed}{s} code
...
\emph{absoluteBoundsCache} in the method in which the pragma is defined. \ab{it also creates a variable in the class doesn't it?}
...
\end{lstlisting}
a new method called \ct{computeMOGraphElementAbsoluteBounds} is created as\del{ is shown below}.\ab{You can remove the code just below. It does not help much}
\begin{lstlisting} 
...
\end{lstlisting}
\end{enumerate}
In this way, the \chg{refactorized}{refactored} cache code is executed with Mondrian.
...
be generated will \del{be} reset \del{mechanism corresponding to} the cache defined
...
a pragma defined as $<$CacheIni\chg{c}{t}ializationPattern: cacheName$>$.
...
\begin{lstlisting} 
MOGraphElement>>cacheCanvas: aCanvas 
	<CacheIni\chg{c}{t}ializationPattern: #cacheForm>  
...
properly in more than 85\% of the methods of the \ct{MOGraphElement} hierarchy. \ab{Does it mean that 85\% of Mondrian methods use a cache?}
...
One of the main priorities during the \chg{refactorization}{refactoring} process was not to affect the performance of the system. 
For this reason a group of benchmarks were measured in order to evaluate the cache performance when a set of nodes and edges are displayed. The variations observed between the system before and after applying refactorings are not significant. That is because, in general, the code after the injection of the caches is the same that the original code before the Mondrian \chg{refactorization}{refactoring}. The details of the benchmarks results can be found in the appendix. 
