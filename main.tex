...
Fig. \ref{fig:Cache-behaviour-delegation.}). However, this kind of
modularization looses performances because the access to the cache
variable not direct. This causes a delay when attempting to access
the values that keep the caches. So, the separation of this concern
is not a trivial problem.%

\ab{Can you give some number, how much slower was this? No need to be very accurate. We were about 10 times slower weren't we?}

\ab{What was the refactoring you did? Figure 1 shows the solution, you first attempt. You just need to refine a bit the figure that shows MOGraphElement with 2 methods, resetCache and bounds for example}
...
it introduced indirections to access to the caches \ins{(\secref{problem})}. Indirections caused a poor performance
that slows down response times of the cache mechanism. For this reason,
...
\chg{of}{to} define the pragma inside the method is in order to allow a better
...
before the execution of the method in which the pragma is defined. \ab{I am wondering whether using text as advices is a good things. how would you write using AspectJ?}
...
\ab{I do not understand this paragraph}\emph{Reset Cache}: In order to refactor this pattern each statement
...
In regard to when the code is injected, it depends on the method that
...
call. Even though the order of calls is changed, the method behavior
