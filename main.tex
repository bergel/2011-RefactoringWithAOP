...
because \ugh{the access to the cache variable is not direct}\sv{No entiendo que es lo que deberia poner en lugar de esto}. This causes
a delay\ab{delay?} \sv{me refiero a que tardaba demasiado tiempo en acceder a la cache por la cantidad de indirecciones}when attempting to access the values that keep the caches.
...
\figref{PatternLocation} shows the distribution of the caches over the main Mondrian classes. In this figure, the methods in which the caches are used are shown. Also, \figref{PatternLocation} shows in which class each cache is defined. As can be seen, the caches are used and defined across the whole class hierarchy. \sv{No estoy seguro de poner esta figura aca. Esta mostrando los patrones y aun no han sido explicados}
...
\end{itemize}
The task of group\ab{I am still confused with group here} \sv{Uso los grupos como una forma de explicar como llegamos a los patrones. Es decir, primero dividi todos los usos de las caches en los tres grupos que explique arriba. Despues encontre los patrones dentro de esos grupos.} the fragments of code of the different caches is
...
The method \ct{cacheCanvas:} is invoked when...\ab{please complete}\sv{Acabo de notar que este metodo solo es usado en los test. ¿Pongo algo relacionado con eso?}
...
Additionally, Table \ref{tab:Cache-Concern-scattering} \ab{something wrong with the ref?}\sv{Why?} presents a
