...
because of additional indirection. This causes
...
are grouped together based on the purpose of its use (eg. saving information, obtaining the data stored, etc.). Each group is associated with different activities:
\begin{itemize}
...
The method \ct{cacheCanvas:} only is invoked during testing in order to verify some characteristics of the caches such as their effectiveness.
...
Additionally, Table \ref{tab:Cache-Concern-scattering} gives the occurrences of each pattern in the MOGraphElement hierarchy,
...
are established. The goal of the refactorization is the extraction of these patterns from the main code without changing the behavior of the system.
...
\emph{absoluteBoundsCache} in the method in which the pragma is defined. \ab{it also creates a variable in the class doesn't it?} \sv{No, the only addition into the original method is the pragma}\ab{Ok, but what happens with the instance variable?}\sv{Sorry, I don't understand. What instance variable?}

\ab{We are here describing the weaving approach} \sv{But the weaving approach is the injection approach (just with another name). Isn't it?}Once that the cache code is extracted into the pragmas, the code to
