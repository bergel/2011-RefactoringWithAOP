...
This paper presents a solution to a maintenance problem we recently faced with while developing the Mondrian application.
Mondrian is an agile visualization engine implemented in Pharo, and is used in more than a dozen projects \ga{podemos mencionar algunos?}. As in many software developments, new requirements of several increasing number of clients impact on design decisions that were hold for years. 
...
The goal of the refactoring is the separation \ga{puedo decir extraction?} of the \emph{Cache
...
repeated when using the caches. An aspect refactoring \ga{podemos decir aspect-based refactoring?} is associated for each found pattern~\cite{Kellens2007}. These code patterns are described in the following subsections.
...
\emph{resetValue} and the initial value the cache has to have \sv{Is something missing here?}\ga{pareciera que son dos valores, por la descripcion}. Typically, the \emph{resetValue} depends on the type of the stored value. It could be \ct{nil}, an empty dictionary, or a particular value (\eg \ct{0@0}).
...
\paragraph{Lazy Initialization.} In some situations it is not relevant to initialize the cache before it is actually needed. This happens when a graphical element is not outside the displayed window \ga{puedo decir: is contained but not displayed in a window?}: no cache initialization is required for a graphical element if the element is not displayed. These caches are relevant only when the user actually sees the element by scrolling the visualization. Typically, the structure of this pattern is: \emph{\^{} cache ifNil:[cache:=newValue]}. Mondrian contains five occurrences of a lazy cache initialization. Consider the \ct{bounds} method:
...
Once the code patterns are identified, we set up strategies to refactor them. The goal of the refactorization is the separation \ga{extraction?} of these patterns from the main code without changing the overall behavior, enforced by an extended set of unit tests.
...
\ct{CachePattern} class contains the methods needed to process annotations (called pragmas in the Pharo terminology). Each subclass overrides \ct{generateMethodWith:} method to perform the source code manipulation.
...
\secref{Pattern-Identification} of the \emph{resetCache} method,
...
in the case of the \emph{bounds} method introduced in the previous
...
This experience has been the opportunity to think again on the implementation of Mondrian. We found one occurrence where a cache variable is not solely used as a cache, but as part of main logic of Mondrian. The method bounds contains an access to \ct{boundsCache}: 
...
All Mondrian memoization implementations have been refactored into a dedicated aspect. \sv{debe completarse ¿no?}
