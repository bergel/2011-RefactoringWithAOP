...
The goal of the refactorization was the separation of the cache concern
from the main class of \emph{Mondrian}: \emph{MOGraphElement} and
its subclasses (\emph{MOEdge}, \emph{MONode}, and \emph{MORoot}).
...
After this initial identification, the fragment of codes in wich the
caches were used were grouped together based on the purpose of its
use. The main general purposes identified were:
\begin{itemize}
...
the following presentation structure is used:
\begin{itemize}
\item Name of the pattern.
\item Brief description of the pattern and typical situations in which it
appears.
\item Number of occurrences.
\item Code Example
\end{itemize}
The purpose of the code examples is to illustrate the occurrences
of each pattern in the proper context. 
...
the \emph{resetValue} is nill or a new of a kind of Dictionary object.
Fourteen ocurrences of this pattern were found in the Mondrian code.
...
is nill. Tipically, the structure of this pattern is: \emph{if (cache==nill)
cache:=newValue. return cache}. Four ocurrences of this pattern were
found in the Mondrian code. Next, the method \emph{bounds} is presented
as an example in which boundsCache is accesed.
...
which a value is asigned to a cache. The struture of the pattern is
only an assignation: cache:=\emph{aValue}. This pattern was found
...
is accesed. The structure of the pattern is the return of the cache:
\emph{return cahe}. This pattern was found in two occasions. Next,
the method \emph{shapeBounds} is presented as an example in which
cacheShapeBounds is accesed.
...
Una vez que describi a todos podria decir cual fue la estrategia de
refactorizacion que seguimos y mostar la solucion para cada uno de
los ejemplos de codigo que puse
