...
Coping with emerging requirements is probably one of the most difficult challenge in software engineering~\cite{Somm00a}. 
...
Mondrian use simple two-dimensions rendering to graphically visualize an arbitrary domain. 
One of strong assumption Mondrian holds is the structure of its multiple cache mechanisms. Mondrian's caches are instance of the memoization technique\footnote{\url{http://www.tfeb.org/lisp/hax.html\#MEMOIZE}}. Sending twice the same message returns the same value if no side effect impact the computation. 
...
We subsequently undertaken a major refactoring of Mondrian's core: caches are not externalized and structured along a class hierarchy.
...
Extending the range of applications for Mondrian invalidate some of the caches. For example \ct{absoluteBoundsCache} has no meaning in the three-dimensional version of Mondrian since the circumscribed rectangle is meaningful only with two dimensions.
...
\paragraph{Significant overhead.} This modularization solely based on delegating message has a sever overhead at execution time
because \chg{the access to the cache variable is not direct}{of additional indirection}. This causes
a \chg{delay when attempting to access the values that keep the caches}{a significant overhead}.
...
\caption{Cache behavior delegation.\label{fig:Cache-behaviour-delegation}\ab{How is absoluteBounds written in that case?}}
...
variables related to the caches and the places where they are used. \ab{say how you identified the caches. Using variable accessors, sender of accessor and mutators I imagine?}The caches are mostly identified by browsing the methods in which the caches variables are referenced and accessed.
...
\figref{PatternLocation} shows the distribution of the caches over the main Mondrian classes. In this figure, the methods in which the caches are used are shown. Also, \figref{PatternLocation} shows in which class each cache is defined. As can be seen, the caches are used and defined across the whole class hierarchy. \sv{No estoy seguro de poner esta figura aca. Esta mostrando los patrones y aun no han sido explicados}\ab{Tiene razon creo. Pone la figura en Section 3.2}

After this initial
identification, the fragment of codes \ugh{in which} the caches are used
are grouped together based on the \ugh{purpose of its use} (eg. saving information, obtaining the data stored, etc.). Each group is associated with different activities:
\begin{itemize}
...
\item Retrieve the cache value: this group obtain the information that is saved in a cache.
\item Store data in the cache: the code fragments grouped here save information into a cache varible such as an instance of a class.
\end{itemize}
\ab{I am wondering whether we need this paragraph at all. I think we can safely remove it. It basically says what comes next.} The task of group\ab{I am still confused with group here} \sv{Uso los grupos como una forma de explicar como llegamos a los patrones. Es decir, primero dividi todos los usos de las caches en los tres grupos que explique arriba. Despues encontre los patrones dentro de esos grupos.} the fragments of code of the different caches is
...
repeated in the use of the caches. For each pattern found a refactoring strategy is associated. These code patterns are described in the following subsections.
...
\paragraph{Reset Cache.} A cache has to be invalidated when its content has to be actualized. We refer to this action as reset. The reset structure is \emph{cache:=resetValue} where
\emph{resetValue} depends on the cache internal structure. Typically,
the \emph{resetValue} depends on the type of the stored value. It could be \ct{nil}, an empty dictionary, or a particular value (\eg \ct{0@0}).
...
\paragraph{Lazy initialization.} In some situations it is not relevant to initialize the cache before it is actually needed. This happens when a graphical element is not outside the window. These caches are relevant only when the user actually see the element by scrolling the visualization. Typically, the structure of this pattern is: \emph{\^{} cache ifNil:[cache:=newValue]}. Mondrian contains five occurrences of a lazy cache initialization. Consider the \ct{bounds} method:
...
The circumscribed rectangle is returned by \ct{computeBoundsFor:} and is performed only when a edge is actually visible (\ct{bounds} is used in \ct{drawOn:}, the rendering method.
...
The method \ct{cacheCanvas:} is invoked when...\ab{please complete}\sv{Acabo de notar que este metodo solo es usado en los test. ¿Pongo algo relacionado con eso?}\ab{Si, prueba}
...
Additionally, Table \ref{tab:Cache-Concern-scattering} \chg{presents a
summary of}{gives} the occurrences of each pattern in the MOGraphElement hierarchy,
...
\ab{I was wondering. Why not to rename this section as ``Memoization as an aspect''? After some discussion with Eric, we are still doing aop, even without AspectJ or an explicit aspect language. We could then use "weaving" instead of "injection"}

Once the code patterns are identified, strategies to refactor them
are established. The goal of the refactorization is the extraction of these patterns from the main code without changing the behavior of the system. \del{The main constraint to refactoring the code is the
preservation of a good performance of the cache mechanism}\ab{Section 2.4 says what are the requirements. Just remove the sentence}.
...
of the cache mechanism. For this reason, this is not a feasible solution\ab{It seems that we are repeating ourselves. Can this paragraph be removed?}.
...
\emph{absoluteBoundsCache} in the method in which the pragma is defined. \ab{it also creates a variable in the class doesn't it?} \sv{No, the only addition into the original method is the pragma}\ab{Ok, but what happens with the instance variable?}

\ab{We are here describing the weaving approach} Once that the cache code is extracted into the pragmas, the code to
...
\paragraph{Importance of the tests} However, in practice, this has not caused any noticeable problem. The extensive test set of Mondrian remained green after the instrumentation.



%: % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
\section{Related Work}\seclabel{relatedWork}
Most of the work in AOP has been focused in Java and AspectJ. An approach called AspectS has been proposed for Smalltalk-Squeak~\cite{Hirschfeld2002}. This is a general purpose AOP language with dynamic weaving. \sv{No estoy seguro de como justificar que no utilizamos AspectS para los caches. ¿Si funciona en squeak funciona en pharo? Una posibilidad es el overhead que implica la definicion de los pointcuts y aspectos y que no se garantizaba la misma performance en la ejecucion} 
