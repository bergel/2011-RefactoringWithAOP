...
\emph{\chg{Return Cache with Precondition Checking}{Lazy initialization}}: this pattern shows
...
\emph{Cache Loaded}: this pattern check\ins{s} \chg{is}{whether} \chg{a cache or a group of them}{one cache or more}
are \chg{loaded}{initialized} or conversely, if they are \ins{not?} nil. So, the structure of the
...
Several alternatives were explored to encapsulate\del{d} the \emph{Cache
...
\emph{$<$ReturnCachePatternForCache: \#cacheShapeBounds$>$} \ab{I suggest to remove all ForCache: from the patternCodeName. I see a redundancy here} indicates
...
a new method called computeMOGraphElementAbsoluteBounds is created. \ab{add example}
\item The code of the original method is copied into the new method. \ab{add example}
...
method contains a call to the new method of the Step 1. \ab{add example}
\end{enumerate}
In this way, the refactorized cache code is executed with Mondrian.
The whole injection mechanism is summarized in Fig. \ref{fig:Injection-mechanism.}.\ab{I would remove the figure. It does not say much}
%
\begin{figure}
\begin{centering}
...
\caption{Injection mechanism.\label{fig:Injection-mechanism.}\ab{I would remove the figure. It is not really helpful.}}
...
this hierarchy is compose\ins{d} \chg{by}{of} the classes \emph{AbstractResetCachePattern},
...
\ab{What about resetElementsToLookup and resetMetricCaches? Shouldn't we talk about them?}
...
\ab{This section is important. Can you provide an example? Why some of the methods cannot be restructured? displayOn: is one of them} With the use of these patterns the \emph{Cache Concern} is refactorized
